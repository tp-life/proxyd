...
func (m *Manager) startServerLocked(cfg config.RemoteConfig) error {
	priv, err := loadOrCreateServerKey(filepath.Join(m.stateDir, serverKeyRelPath))
	if err != nil {
		return err
	}
...
// loadOrCreateServerKey 读取持久化密钥；不存在时生成新密钥并以 0600 落盘。
func loadOrCreateServerKey(path string) (priv key.NodePrivate, err error) {
