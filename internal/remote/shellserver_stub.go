...
//   - shellUser: string，不支持平台不使用该值。
//
// 返回值说明：func(net.Conn) 恒为 nil；error 恒为平台不支持错误。
//
// 错误情况：无条件返回 errShellTerminalUnsupported。
func localShellSSHHandler(_, _ string) (func(net.Conn), error) {
	return nil, errShellTerminalUnsupported
}

// configuredShellSSHHandler 为不支持的平台提供一致的配置入口。
// 参数说明：string 为状态目录，bool 为公钥开关，[]config.RemoteSSHKey 为授权集合，
// 最后一个 string 为 shell-user，均不使用。
// 返回值说明：nil 处理器与平台不支持错误。
// 错误情况：始终返回 errShellTerminalUnsupported，不创建任何监听或 shell。
func configuredShellSSHHandler(_ string, _ bool, _ []config.RemoteSSHKey, _ string) (func(net.Conn), error) {
	return nil, errShellTerminalUnsupported
}
