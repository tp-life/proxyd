...
// shellUser 为 string，remote.shell-user 配置原值，诊断同样以降权会话用户运行。
// 返回值说明：无，环境字段与进度标记通过 SSH 输出，退出码由 shell 回传。
// 错误情况：会话用户解析失败、未申请 PTY、shell 启动失败时返回非零；客户端断开沿用会话清理逻辑。
func shellDiagnosticHandler(sess ssh.Session, shellUser string) {
	...
	runShellDiagnostic(sess, su)
}

// runShellDiagnostic 为已经确认的会话身份构造交互环境探针。
// 参数说明：sess 为 ssh.Session；su 为 *sessionUser，必须由服务端解析，不接受客户端用户切换。
// 返回值说明：无，探针输出通过原始会话传回。
// 错误情况：启动文件阻塞时由客户端超时关闭传输，服务端沿用原有子进程清理。
func runShellDiagnostic(sess ssh.Session, su *sessionUser) {
	...
	cmd := newShellDiagnosticCommand(su, script)
