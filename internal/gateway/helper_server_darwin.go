...
const (
	// legacyOwnerUIDEnv 是旧版独立 gateway-helper plist 记录的属主 UID 环境变量；
	// 统一助手经 privhelper.OwnerUIDEnv 下发，本变量仅用于旧 plist 过渡期回退。
	legacyOwnerUIDEnv = "PROXYD_GATEWAY_OWNER_UID"
...
// （privhelper.OwnerUIDEnv），回退旧版独立 gateway-helper 变量（旧 plist 过渡期）。
//
// 参数：无。
//
// 返回值：
