...
// 参数：args 为 []string，支持 -c、list、proxy|remote on|off|retry；返回 error，参数或 API 错误原样上报。
func cmdModules(args []string) error {
	...
	if len(rest) == 0 || len(rest) == 1 && rest[0] == "list" {
		...
		if len(rest) != 2 || (rest[0] != "proxy" && rest[0] != "remote") || (rest[1] != "on" && rest[1] != "off" && rest[1] != "retry") {
			return fmt.Errorf("用法: proxyd modules [-c 配置] list | proxy|remote on|off|retry")
		}
		if rest[1] == "retry" {
