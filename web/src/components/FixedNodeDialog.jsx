/** 默认出口选择弹窗：仅维护候选选择，配置写入与状态刷新交给应用入口。 */
...
const EXIT_SPECIALS = [
  { value: "AUTO", label: "自动最快", detail: "由 AUTO 测速组选择当前延迟最低的节点", badge: "自动选择", icon: Sparkles },
  { value: "DIRECT", label: "直连", detail: "不经过任何节点，直接访问目标", badge: "不走代理", icon: Globe2 },
];

/**
 * FixedNodeDialog 以关键词和来源联合筛选，选择规则模式的默认出口（PROXY 组选中项）。
 *
 * 功能说明：
 * 默认出口由内置 PROXY 组的持久化选中项表达，取值是节点名或保留名 AUTO / DIRECT
 * （不是节点 key），因此候选与当前值都按节点名匹配；AUTO 需要至少一个可用节点，
 * DIRECT 恒可选。确认成功后弹窗关闭，失败保留选择并提示重试。
 *
 * 参数说明：
 * - nodes: Array<object> 节点快照。
 * - currentNode: string 当前默认出口值（节点名 / AUTO / DIRECT），空串表示未选择。
 * - canAuto: boolean，是否存在可用节点（决定 AUTO 是否可选）。
 ...
 */
export function FixedNodeDialog({ nodes, currentNode, canAuto = true, triggerElement, onSelect, onClose }) {
  ...
  const candidateLabel = selected === "AUTO" ? "自动最快" : selected === "DIRECT" ? "直连" : candidateNode?.name || "";
  const candidateValid = selected === "DIRECT" || (selected === "AUTO" && canAuto) || Boolean(candidateNode);
  ...
  const visibleSpecials = source
    ? []
    : EXIT_SPECIALS.filter((option) => `${option.label} ${option.detail}`.toLocaleLowerCase().includes(keyword));
  ...
  return <Dialog open onOpenChange={changeOpen}>
    <DialogContent className="fixed-node-dialog" showClose={!saving} onCloseAutoFocus={restoreFocus} aria-busy={saving}>
      <header className="dialog-header">
        <DialogTitle>选择默认出口</DialogTitle>
        <DialogDescription>规则模式未命中规则的流量出口；TUN 与网关流量同样经过它。</DialogDescription>
      </header>
      ...
      <div className="fixed-node-list" aria-label="候选出口">
        {visibleSpecials.map((option) => {
          ...
          const disabled = saving || (option.value === "AUTO" && !canAuto);
          return <button key={option.value} type="button" className={classNames("fixed-node-card", selected === option.value && "selected")} aria-pressed={selected === option.value} disabled={disabled} onClick={() => { setSelected(option.value); setError(""); }}>
            ...
            <span className="fixed-node-card-status"><span className="delay-muted">{option.value === "AUTO" && !canAuto ? "暂无可用节点" : option.badge}</span>{option.value === currentNode && <Badge variant="outline">当前默认出口</Badge>}</span>
          </button>;
        })}
        {visibleNodes.map((node) => <button key={node.key} type="button" className={classNames("fixed-node-card", selected === node.name && "selected")} aria-pressed={selected === node.name} disabled={saving || !node.alive} onClick={() => { setSelected(node.name); setError(""); }}>
          ...
          <span className="fixed-node-card-status"><span className={delayClass(node)}>{node.alive ? formatDelay(node) : "不可用"}</span>{node.name === currentNode && <Badge variant="outline">当前默认出口</Badge>}</span>
        </button>)}
        {!visibleSpecials.length && !visibleNodes.length && <p className="fixed-node-empty">{source ? "没有匹配的节点，请调整关键词或来源筛选。" : nodes.length ? "没有匹配的出口，请调整关键词。" : "暂无节点，请先添加节点或同步订阅；也可以直接选择 AUTO 或 DIRECT。"}</p>}
      </div>
      <div className="fixed-node-feedback" aria-live="polite">
        {error ? <p role="alert">{error}</p> : <p>{candidateValid ? `已选择：${candidateLabel}` : "请选择可用节点、AUTO 或 DIRECT。"}</p>}
      </div>
