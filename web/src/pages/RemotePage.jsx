...
export function RemotePage({
  ...
  const forwardColumns = useMemo(
          ...
          <div className="grid justify-items-start gap-1">
            ...
            {row.remote_port === 22 && row.enabled && (
              <button
                ...
                title="复制 SSH 命令"
                type="button"
                onClick={() => copyText(`ssh ${parseListen(row.listen).host} -p ${parseListen(row.listen).port}`, "SSH 命令已复制")}
              >
                ssh {parseListen(row.listen).host} -p {parseListen(row.listen).port}<Copy size={12} aria-hidden="true" />
              </button>
            )}
          </div>
        ),
    ...
    [copyText, removeForward, toggleForward],
  );
  ...
  return (
    <div className="stack">
      ...
      {initialLoading ? (
        ...
        <>
          <section className="panel">
            <PanelTitle title="服务状态" detail="tailcat 隧道服务的开关与运行状态" />
          ...
          <section className="panel">
            <PanelTitle title="暴露端口" detail="这些是本机端口，持有 token 的对端可经隧道访问（如 22 用于 SSH）" />
            <form className="form-grid" onSubmit={submitServePort}>
              <Field label="端口" hint="1-65535 的本机端口">
                <input aria-label="新增暴露端口" max="65535" min="1" type="number" value={serveInput} onChange={(event) => setServeInput(event.target.value)} placeholder="例如：22" />
              </Field>
            ...
            {serve.length === 0 ? (
              <EmptyState title="尚未暴露端口" detail="添加本机端口后，持有 token 的对端即可经隧道访问该端口。" />
            ) : (
          ...
          <section className="panel">
            <PanelTitle title="远程设备" detail="保存对端 token，供本地转发与 SSH 连接使用" />
            <form className="form-grid" onSubmit={submitRemote}>
              <Field label="名称">
              ...
              <Field label="对端 token" hint="对端 proxyd 状态卡中的完整 token">
                <input aria-label="远程设备 token" className="mono-input" value={remoteForm.token} onChange={(event) => setRemoteForm((current) => ({ ...current, token: event.target.value }))} placeholder="tc..." />
              </Field>
              <Button className="form-submit" type="submit"><Plus size={16} aria-hidden="true" /><span>添加设备</span></Button>
            </form>
            {remotes.length === 0 ? (
              <EmptyState title="暂无远程设备" detail="添加对端 token 后，即可建立到该设备的本地转发或 SSH 连接。" />
            ) : (
              <Table
          ...
          <section className="panel">
            <PanelTitle title="本地转发" detail="把本机监听地址经隧道转发到远端设备的指定端口" />
            <form className="form-grid" onSubmit={submitForward}>
              <Field label="名称">
              ...
              {forwardForm.remoteSource === "" && (
                <Field label="远端 token" hint="以 tc 开头的完整 token">
                  <input aria-label="远端 token" className="mono-input" value={forwardForm.remoteToken} onChange={(event) => setForwardForm((current) => ({ ...current, remoteToken: event.target.value }))} placeholder="tc..." />
                </Field>
              )}
              <Field label="远端端口" hint="对端设备上的目标端口">
            ...
            {forwards.length === 0 ? (
              <EmptyState title="暂无本地转发" detail="添加转发后，访问本机监听地址即相当于访问远端设备的对应端口。" />
            ) : (
              <Table
                className="data-table mt-3"
 ...
 */
function PeerConnectDialog({ name, forwards, fetchPeerToken, createSSHForward, toggleForward, copyText }) {
    ...
    <>
      ...
      <div className="dialog-form">
        ...
        <div className="grid gap-2">
          ...
          {activeForward ? (
            <>
              <SnippetRow label="SSH 登录" text={`ssh ${login}${host} -p ${port}`} onCopy={(text) => copyText(text, "SSH 命令已复制")} />
        ...
        <div className="grid gap-2">
          ...
          {tokenState.loading ? (
            ...
            <>
              <SnippetRow label="proxyd SSH" text={`proxyd ssh ${token}`} onCopy={(text) => copyText(text, "命令已复制")} />
